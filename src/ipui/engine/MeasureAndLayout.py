...
class MeasureAndLayout:
    ...
    def measure_node(self, node):
        ...
        if node.width_flex > 0: node.width_minimum = node.frame_size
        if node.height_flex > 0: node.height_minimum = node.frame_size #added 3/6

    def cap_scrollable_min(self, node):
        """Scrollable containers keep content size for scroll math but report minimal min to flex."""
        if not node.scrollable: return             #If not scrollable, this doesn't apply
        if node.horizontal:
            node.content_size = node.width_minimum
            node.width_minimum = node.frame_size
        else:
            node.content_size = node.height_minimum
            node.height_minimum = node.frame_size
