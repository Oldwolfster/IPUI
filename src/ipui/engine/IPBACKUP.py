...
class IP:
    ...
    def __init__(self):
        ...
        self.pane_rect          = None      # the pane widget's rect — where your content lives
        self.content_rect       = None      # usable area inside pane after pad/border
    ...
    def set_pane_contextOld(self, pane, pane_name, is_active, pane_widget=None):
        ...
        if pane_widget and pane_widget.rect:
            self.pane_rect    = pane_widget.rect
            frame             = pane_widget.frame_size
            self.content_rect = pane_widget.rect.inflate(-frame, -frame)
        else:
            self.pane_rect    = None
    ...
    def set_pane_context(self, pane, pane_name, is_active, pane_widget=None):
        ...
        if pane_widget and pane_widget.rect:
            ...
            self.pane_rect    = None
        self.content_rect     = self.find_canvas_rect()
    ...
    def local_to_screen(self, x, y):
        ...
        if self.pane_rect:
            return (x + self.pane_rect.left, y + self.pane_rect.top)
        return (x, y)
    ...
    def mouse_inside_pane(self):
        ...
        return self.pane_rect and self.pane_rect.collidepoint(self.mouse_pos)

    def mouse_inside_content(self):
        """Is the mouse inside the pane's content area?"""
        return self.content_rect and self.content_rect.collidepoint(self.mouse_pos)
    ...
    def mouse_local_pos(self, widget=None):
        ...
        if self.pane_rect:
            return (self.mouse_x - self.pane_rect.left,
                    self.mouse_y - self.pane_rect.top)
        return (self.mouse_x, self.mouse_y)

    def mouse_local_x(self, widget=None):
        """Mouse x relative to widget or pane left edge."""
        ref = widget.rect if widget and widget.rect else self.pane_rect
    ...
    def help_topic(self, topic):
        ...
        lookup = {
            ...
            "pane_rect":    self.help_section_geometry,
...
"""
    ...
    def help_section_geometry():
        return """
═══ GEOMETRY ═══════════════════════════════
  ip.pane_rect        Rect of the current pane widget
  ip.rect_pane     Usable area inside pane (after pad/border)
