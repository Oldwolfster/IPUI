...
class EZ_Pane(_BaseTab):
    ...
    def workbench(self, parent) -> None:
        ...
        Heading(sub, "Layer:")
        row = Row(sub)
        btn_add_layer = Button(row, "Add Layer", color_bg=Style.COLOR_BUTTON_CTA)
        btn_add_layer.on_click_me(self.add_layer)
        btn_rem_layer = Button(row, "Remove Layer", color_bg=Style.COLOR_BUTTON_DANGER)
        btn_rem_layer.on_click_me(self.remove_layer)

        sub = CardCol(parent)
        Heading(sub, "Neurons:")
        row = Row(sub)
        btn_add_neuron = Button(row, "Add Neuron", color_bg=Style.COLOR_BUTTON_CTA)
        btn_add_neuron.on_click_me(self.add_neuron)
        btn_rem_neuron = Button(row, "Remove Neuron", color_bg=Style.COLOR_BUTTON_DANGER)
    ...
    def preview(self, parent) -> None:
        ...
        btn_forge = Button(header, "Forge It!", color_bg=Style.COLOR_BUTTON_CTA)  # NEW
    ...
    def card_percy_btn(self, parent) -> None:
        ...
        sub = CardCol(parent)
        btn = Button(sub, "Percy's Story", color_bg=Style.COLOR_BUTTON_CTA)
