...
class Pipe(_BaseTab, XGBoostMixin):
    ...
    def top_section(self, parent):
        ...
        TextBox(header, initial_value="2026-03-27", name="txt_start_date") #date_relative_to_today
        Body(header, "to:")
        TextBox(header, initial_value="2026-03-27", name="txt_end_date")
        Spacer(header)
    ...
    def build_one_card(self, parent, tbl):
        ...
        body_rows = Body(card, f"Rows:   {rows:,}")
        rng = f"{MgrDT.gd_to_iso(mn)}  to  {MgrDT.gd_to_iso(mx)}"
        body_range = Body(card, f"Range:  {rng}")
        btns = Row(card)
        Button(btns, "Refresh"  ,   on_click=lambda: self.refresh_table(tbl, body_rows, body_range))
    ...
    def refresh_table(self, tbl, body_rows, body_range):
        ...
        body_rows.text = "Rows:   refreshing..."  # paints THIS frame
        self.ip.after_paint(self.refresh_table_now, tbl, dates, body_rows, body_range)

    # Pipe.py  method: refresh_table_now  Update: runs after flip; updates THIS card only — no pane rebuild
    def refresh_table_now(self, tbl, dates, body_rows, body_range):
        ...
        self.update_card_stats(tbl, body_rows, body_range)

    # Pipe.py  method: update_card_stats  New: re-read counts and update one card's labels in place
    def update_card_stats(self, tbl, body_rows, body_range):
        rows   = BB.row_count (tbl)
        mn, mx = BB.date_range(tbl)
        rng    = f"{MgrDT.gd_to_iso(mn)}  to  {MgrDT.gd_to_iso(mx)}"
        body_rows .set_text(f"Rows:   {rows:,}")
        body_range.set_text(f"Range:  {rng}")
    ...
    def match_columnsOLD(self, view, tbl):
    ...
    def match_columns(self, view, tbl):
        view_cols   = set(r[1] for r in BB.query(f"PRAGMA table_info({view})"))
        target_info = BB.query(f"PRAGMA table_info({tbl})")
        target_cols = [r[1] for r in target_info]
        common      = [c for c in target_cols if c in view_cols]
        extras      = view_cols - set(target_cols)
        missing     = set(target_cols) - view_cols
