...
class Breakout(_BaseTab):
    ...
    def ip_setup(self, ip):
        ...
        self.level          = 1
        self.bricks         = []
        ip.state.add        ("DEMO_PLAYING"  , self.state_demo_playing)
        ip.state.add        ("DEMO_LEVEL_UP" , None, "DEMO_PLAYING", 1.5)
        ip.state.add        ("READY"         , self.state_ready)
        ip.state.add        ("PLAYING"       , self.state_playing)
        ip.state.add        ("LEVEL_UP"      , None, "READY", 1.5)
    ...
    def next_level(self):
        ...
        self.reset_ball()
        self.update_hud()
        if self.ip.state.current and self.ip.state.current.startswith("DEMO"):
            self.ip.state.go("DEMO_LEVEL_UP")
        else:
    ...
    def build_hud(self):
        ...
        Spacer(row)
        Title(row, " ", name="lbl_level", glow=True)
        Spacer(row)
        Title(row, " ", name="lbl_lives", glow=True)
        self.update_hud()
    ...
    def update_hud(self):
        ...
        if lbl:
            lbl.set_text("DEMO" if self.ip.state.current and self.ip.state.current.startswith("DEMO") else f"Lives: {self.lives}")
    ...
    def ip_draw(self, ip):
        #print(f"breakout pane ={ip.pane_name}")
        if not ip.rect_pane:
            return
        self.draw_arena(ip)
        self.draw_bricks(ip)
    ...
    def aim_ball(self):
        if len(self.bricks) != 1:    return
        brick = self.bricks[0]
        tx = brick["x"] + brick["w"] / 2
        ty = brick["y"] + brick["h"] / 2
        dx = tx - self.ball_x
        dy = ty - self.ball_y
        dist = (dx ** 2 + dy ** 2) ** 0.5
        if dist == 0:                return
        speed = (self.ball_dx ** 2 + self.ball_dy ** 2) ** 0.5
        self.ball_dx = dx / dist * speed
        self.ball_dy = dy / dist * speed
    ...
    def check_ball_lost(self):
        ...
        if self.lives <= 0:
            ...
            return
        self.reset_ball()
        if not self.ip.state.current.startswith("DEMO"):
            self.ip.state.go("READY")
    ...
    def draw_paddle(self, ip):
        ...
        pygame.draw.rect(ip.surface, Style.COLOR_BUTTON_ACCENT,
    ...
    def draw_state_message(self, ip, msg):
        ...
        color     = Style.COLOR_BUTTON_ACCENT
