...
class WidgetExplorer(_BaseTab):

    def pane1(self,parent):
    ...
    def explorer_menu(self, parent):
        ...
        card = Card(parent, scrollable=True, height_flex=True)
    ...
    # ══════════════════════════════════════════════════════════════
# WidgetExplorer.py  demos batch 1
    ...
    def demo_020_buttons(self, parent):
        ...
        Button  ( parent, "Default Button")
        Button  ( parent, "Call To Action",  color_bg=Style.COLOR_BUTTON_CTA)
        Button  ( parent, "Danger",          color_bg=Style.COLOR_BUTTON_DANGER)
        Button  ( parent, "Secondary",       color_bg=Style.COLOR_BUTTON_SECONDARY)
        Button  ( parent, "Warning",         color_bg=Style.COLOR_BUTTON_WARNING)
    ...
    def demo_030_textbox(self, parent):
        ...
        TextBox ( parent, initial_value="Pre-filled",  width_flex=True)
    ...
    def demo_080_grid(self, parent):
        ...
        grid    = PowerGrid(parent, height_flex=True)
        grid.set_data(data)
...
# WidgetExplorer.py  demos batch 2 — intermediate and advanced patterns
    ...
    def demo_100_dropdown(self, parent):
        ...
        dd      = DropDown(parent, text="Pick a color", data=items, width_flex=True)
    ...
    def demo_120_single_select(self, parent):
        ...
        sel     = SelectionList(parent, text="Size", data=items,
                                single_select=True, height_flex=True)
    ...
    def demo_130_textarea(self, parent):
        ...
        ta      = TextArea(parent, height_flex=True, initial_value="SELECT *\nFROM users\nWHERE active = 1")
    ...
    def demo_210_pipeline_read_write(self, parent):
        ...
        Button  ( parent, "Read Count",     color_bg=Style.COLOR_BUTTON_SECONDARY,
    ...
    def demo_500_justify(self, parent):
        ...
        row1    = Row(parent, width_flex=True, justify_spread=True)
        Button  ( row1,   "A")
        Button  ( row1,   "B")
        Button  ( row1,   "C")
        Body    ( parent, "justify_center=True:")
        row2    = Row(parent, width_flex=True, justify_center=True)
        Button  ( row2,   "X")
        Button  ( row2,   "Y")
    ...
    def demo_510_fit_content(self, parent):
        ...
        Button  ( parent, "I stretch to fill", width_flex=True)
    ...
    def demo_520_nested_cards(self, parent):
        ...
        row     = Row(outer, width_flex=True)
        info    = CardCol(row, width_flex=True)
        Heading ( info,   "Contact")
        Body    ( info,   "alice@example.com")
        Body    ( info,   "555-0123")
        stats   = CardCol(row, width_flex=True)
        Heading ( stats,  "Stats")
    ...
    def demo_530_dynamic_list(self, parent):
        ...
        self.demo_list_card = Card(parent, scrollable=True, height_flex=True)
        self.demo_list_items = ["Alpha", "Bravo", "Charlie"]
        self.rebuild_demo_list()
        row     = Row(parent, width_flex=True)
        self.demo_list_input = TextBox(row, placeholder="New item...", width_flex=True,
    ...
    def rebuild_demo_list(self):
        ...
        for i, item in enumerate(self.demo_list_items):
            row     = CardRow(self.demo_list_card, width_flex=True, justify_spread=True)
            Body    ( row,    f"{i + 1}. {item}", width_flex=True)
    ...
    def demo_540_row_col_nesting(self, parent):
        ...
        top     = CardCol(right, width_flex=True)
        Title   ( top,    "Main Content")
        Body    ( top,    "This is a classic sidebar + main layout.")
        bot     = Row(right, width_flex=True)
        CardCol ( bot, width_flex=True).tap(lambda c: Body(c, "Footer Left"))
        CardCol ( bot, width_flex=True).tap(lambda c: Body(c, "Footer Right"))
    ...
    def demo_630_lifecycle_hooks(self, parent):
        ...
        Body    ( parent, "Four hooks give you full control of the game loop:")
        card    = CardCol(parent)
    ...
    def demo_710_construction_is_attachment(self, parent):
        ...
        Body    ( parent, "An entire class of bugs — eliminated by design.")
    ...
    def demo_900_philosophy(self, parent):
        ...
        Banner  ( parent, "Easy to get right.\nHard to get wrong.", text_align=CENTER)
        card    = CardCol(parent)
