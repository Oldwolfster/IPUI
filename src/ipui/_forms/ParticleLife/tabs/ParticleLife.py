...
class ParticleLife(_BaseTab):
    ...
    def particle_life(self, parent):
        root = Card(parent, flex_height=1)

        buttons = CardRow(root, flex_width=1, justify_spread=True)
        ...
        header = CardRow(saves_card, flex_width=1, justify_spread=True)
        ...
        for save_path in self._list_saves():
            row = CardRow(saves_card, flex_width=1, justify_spread=True)
            txt = TextBox(row, initial_value=save_path.stem, flex_width=1)
            ...
            #Button(row, "Delete", color_bg=Style.COLOR_BUTTON_DANGER,   on_click=lambda p=save_path: self._delete_save(p))
            Button(row, "Load", color_bg=Style.COLOR_BUTTON_CTA,
                   on_click=lambda t=txt: self._load_save(t.private_path))  # NEW
            Button(row, "Delete", color_bg=Style.COLOR_BUTTON_DANGER,
                   on_click=lambda t=txt: self._delete_save(t.private_path))  # NEW

    def _rename_save(self, txt, new_name):
        new_name = new_name.strip()
        if not new_name:
            return
        old_path = txt.private_path
        print(f"RENAME: {old_path.name} → {new_name}.json")
